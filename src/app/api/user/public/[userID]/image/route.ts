...
// using client components messes up your access to clerk, for obvious reasons. And, so, I thought,
// hey, maybe I could just store their image url inside the db, but, then I felt that that was a bit too
// abusive, and so, you know what, wouldn't it be cool if we just, you know, fetched the info we needed for that
// from the API. That would have been trivial with React, I could have just sent it in the response, but, I wanted to avoid that.
// In retrospect, now that this is an API call, I could have probably just added this as a transform into the GET route, maybe I'll
// do that.
